...
This is a **bridge repo** about the **Model Context Protocol (MCP)**, adjacent web standards (`llms.txt`, `.well-known`), **Claude Code Routines** (cloud-scheduled autonomous Claude Code sessions), and **Claude Managed Agents** (the Anthropic Platform's hosted agent harness). It is a knowledge / documentation repository, not a software project — there is no build, lint, or test pipeline. The work here is:
